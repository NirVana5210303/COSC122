...
def quicksort_helper(values, left, right,style):
    ...
    Sorts the portion of values between left and right in-place.
    """
...
def pivot_index_Mo3(values, left, right):
    """
    Returns the median of the index to the item that is the median of the left, right and middle value in the list. The return value should normally be either left, right or middle.
...
def quicksort_range(values, start, end, style='left-pivot'):
    ...
    if len(copy_of_list)==1:
        ...
        # Quicksort the copy of the list 
        quicksort_range_helper(copy_of_list, 0, len(copy_of_list)-1, start, end, style)
        return copy_of_list
...
def quicksort_range_helper(values, left, right, start, end, style):
    ...
    Sorts the portion of values between left and right, but only if they
    overlap with the start:end range.
    """
