...
import unittest, merge_binary_2way, eventlib
...
class TestMergeBinary2Way(unittest.TestCase):
  def test_merge_events(self):
    ...
    self.assertEqual(merge_binary_2way.merge_events(c0, c1), merged)
    self.assertEqual(merge_binary_2way.merge_events(c1, c0), merged)


  def test_binary_merge(self):
    'Ensure that the binary merge is working correctly'
    c0 = [event1, event2]
    c1 = [event3]
    c2 = [event4]

    calendars = [c0, c1, c2]
    self.assertEqual(merge_binary_2way.binary_merge(calendars), [[event1, event2, event3, event4]])

    c3 = [event1a]
    calendars = [c0, c1, c2, c3]
    self.assertEqual(merge_binary_2way.binary_merge(calendars), [[event1, event2, event3], [event1a, event4]])
  ...
  def test_merge_events_same_start_timestamp(self):
    ...
    self.assertEqual(merge_binary_2way.merge_calendars([c0, c1]), merged)
  ...
  def test_efficiency(self):
    ...
    merge_binary_2way.counter.reset()
    self.assertEqual(merge_binary_2way.merge_calendars([events_a, events_b, events_c]), merged)
    self.assertEqual(merge_binary_2way.counter.count, 479)
